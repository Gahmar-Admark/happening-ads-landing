...
export default function AboutBanner() {
  ...
  const fetchBanner = async () => {
    try {
      ...
      const serviceData = res.data.find(item => item.category === 'SERVICES');
      setServiceBanner(serviceData);
    } catch (error) {
      ...
      {loading ? (
        ...
        serviceBanner && (
          <>
            <Image
              className="img-fluid"
              src={serviceBanner.image}
              alt={serviceBanner.altTag || 'Service Banner'}
              width={1920}
              height={400}
            />
            <div className="container h-100 position-absolute caption_box">
              <div className="row h-100">
                <div
                  ...
                  data-aos-delay="200"
                >
                  <h1 className="title_heading">{serviceBanner?.title}</h1>
                </div>
              </div>
            </div>
          </>
