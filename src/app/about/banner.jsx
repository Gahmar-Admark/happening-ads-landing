...
export default function AboutBanner() {
  ...
  const fetchBanner = async () => {
    try {
      ...
      const aboutData = res.data.find(item => item.category === 'ABOUT');
      setAboutBanner(aboutData);
    } catch (error) {
        ...
        aboutBanner && (
          <>
            <Image
              ...
              alt={aboutBanner.altTag || 'About Banner'}
              width={1920}
              height={400}
            />
            <div className="container h-100 position-absolute caption_box">
              <div className="row h-100">
                <div
                  ...
                  data-aos-delay="400"
                >
                  <h1 className="title_heading">{aboutBanner?.title}</h1>
                </div>
              </div>
            </div>
          </>
